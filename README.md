...
| `Pack(pose, ff=None, n_steps=2000, T_start=10.0, T_end=0.1, patience=400, seed=None, ex=1)`      | Sidechain repacking via simulated annealing over the Rotamer Library ensemble at each residue's current backbone (φ, ψ). Choose the `ff` score function or force field, run for `n_steps` steps, geometric cooling from `T_start` to `T_end`. `ex=1` also samples sub-rotamers at ±1 standard deviation about each library mean, matching Rosetta's `-ex1`; `ex=0` uses the means alone. Early-exit if no acceptance occurs in `patience` consecutive steps, and control random seed using `seed`. Returns `(E_final, log)` where `log` carries `'energies'`, `'temperatures'`, `'accepts'` (bool array of accept/reject per step), `'best_E'`, `'steps_run'`, `'stopped_early'` (True if it exited before exhausting `n_steps`), `'restored'` (True if the returned pose is the best-scoring one seen rather than the last), `'n_residues'` (count of repackable residues) |
| `Anneal(pose, ff=None, n_steps=10000, T_start=2000.0, T_end=10.0, sigma_small=5.0, sigma_large=30.0, p_large=0.2, p_shear=0.5, target_acc=0.30, adapt_window=100, seed=None, box=None)`               | Simulated annealing over backbone φ/ψ with two Metropolis move types. Choose the `ff` score function or force field. Each step picks a small (adaptive `sigma_small`) or large (fixed `sigma_large`) Gaussian perturbation; `sigma_small` is updated by Robbins-Monro every `adapt_window` small moves to track `target_acc` ~ 0.30. Geometric cooling from `T_start` to `T_end`. Returns `(E_best, log)` with `'energies'`, `'temperatures'`, `'accepted'`, `'move_types'` (0=single, 1=shear, 2=invalid), `'sigma_history'`, `'best_step'`. The pose is left at the lowest-energy frame |
