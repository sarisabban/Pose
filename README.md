...
E_dock, log = Dock(receptor, ligand, cpu=8)       # Dock a Molecule into a receptor Pose with AutoDock Vina's function
```
...
| `Clash(pose, other=None, overlap=0.4, hydrogens=False, separation=4, hbonds=True, radii=None, verbose=False)` | Finds every pair of atoms whose Bondi van der Waals spheres overlap by more than `overlap` Å, within one `Pose` or `Molecule`, or between `pose` and `other` when a second one is given (receptor vs ligand, chain vs chain). Pairs closer than `separation` bonds in the bond graph are exempt, so 1-2, 1-3 and 1-4 pairs are never counted, in linear, cyclic, disulfide-linked and small-molecule structures alike; `hbonds=True` exempts N/O pairs at ≥ 2.6 Å, the geometry of a hydrogen bond. `hydrogens=False` examines heavy atoms only; `radii={'ZN': 1.2}` overrides an element's radius; `verbose=True` prints the list. Returns a list, worst overlap first, of `(A, B, distance, overlap)` in Å, where `A` and `B` are `[residue name, residue index, atom name, atom index]` for a protein or nucleic acid and `[atom name, atom index]` for a `Molecule`; the number of clashes is `len()` of the list |
...
| `Rotamers(10, pose)`                                                                                          | Sets one residue's side chain to the most probable rotamer given its backbone. No energy is used, so neighbours may clash. Handles D-amino acids automatically |
| `Pack(pose, ff=None, n_steps=2000, T_start=10.0, T_end=0.1, patience=400, seed=None, ex=1)`                   | Repacks every side chain of a structure givens its backbone. Choose the `ff` score function or force field, run for `n_steps` steps, geometric cooling from `T_start` to `T_end`. `ex=1` also samples sub-rotamers at ±1 standard deviation about each library mean, matching Rosetta's `-ex1`; `ex=0` uses the means alone. Early-exit if no acceptance occurs in `patience` consecutive steps, and control random seed using `seed`. Returns `(E_final, log)` where `log` carries `'energies'`, `'temperatures'`, `'accepts'` (bool array of accept/reject per step), `'best_E'`, `'steps_run'`, `'stopped_early'` (True if it exited before exhausting `n_steps`), `'restored'` (True if the returned pose is the best-scoring one seen rather than the last), `'n_residues'` (count of repackable residues) |
| `Anneal(pose, ff=None, n_steps=10000, T_start=600.0, T_end=250.0, sigma_small=5.0, sigma_large=30.0, p_large=0.2, p_shear=0.5, target_acc=0.30, adapt_window=100, seed=None, box=None, alg='md', dt_fs=2.0, friction_ps=1.0)`               | Simulated annealing to explore backbone φ/ψ shapes. Choose the `ff` score function or force field. Each step picks a small (adaptive `sigma_small`) or large (fixed `sigma_large`) Gaussian perturbation; `sigma_small` is updated by Robbins-Monro every `adapt_window` small moves to track `target_acc` ~ 0.30. Geometric cooling from `T_start` to `T_end`, `alg` is `mc` monte carlo or `md` molecular dynamics, where md is more accurate but only works with a force field, mc works with a both a force field and a score function. Returns `(E_best, log)` with `'energies'`, `'temperatures'`, `'accepted'`, `'move_types'` (0=single, 1=shear, 2=invalid), `'sigma_history'`, `'best_step'`. The pose is left at the lowest-energy frame |
| `Minimise(pose, ff=None, max_steps=500, ftol=1.0, alg='lbfgs', dt_fs=0.5, dt_max_fs=1.0, step_max=0.2, etol=1e-6, stall_k=10, box=None)`                                                                                                    | Relaxes all atoms downhill into the nearest local energy minimum. Moves torsions, toramers, bond lengths, and bond angles. A step limiter that bounds per-atom displacement to `step_max` in Å. `ftol` is the convergence threshold on max\|force\| in kJ/mol/Å, `alg` can be `lbfgs` or `fire`, `dt_fs` is the initial integration step in fs and `dt_max_fs` the adaptive ceiling; `etol` and `stall_k` trigger early stop after K consecutive stalled energy steps. Returns `(final_E, log)` where `log` carries `'energies'`, `'fmax'`, `'max_step'`, `'converged'`, `'n_steps'` |
| `MolecularDynamics(pose, ff=None, n_steps=1000, dt_fs=2.0, T=300.0, thermostat='nve', friction_ps=1.0, constraints='hbonds', shake_tol=1e-8, shake_max=100, seed=None, trajectory_every=0, box=None)`                                       | Run a molecular dynamics protocol on a pose. Initial velocities are sampled from Maxwell-Boltzmann at `T`. `thermostat='nve'` runs energy-conserving dynamics; `thermostat='langevin'` runs the BAOAB stochastic splitting at temperature `T` with friction `friction_ps` ps⁻¹. `constraints='hbonds'` enables vectorised SHAKE/RATTLE on every X–H bond (target lengths read from `database.json['Energy Parameters']`), making `dt_fs=2.0` stable; `constraints='none'` disables them. `trajectory_every=k` saves a coordinate snapshot every k steps. Returns `(final_E, log)` with `'energies'`, `'kinetic'`, `'temperatures'`, `'frames'`, `'n_constraints'`, `'dof'` |
| `Dock(pose1, pose2=None, ff=None, centre=None, size=25.0, spacing=0.5, maps=None, chain=None, n_runs=8, n_steps=None, n_poses=9, T=None, min_rmsd=1.0, energy_range=3.0, polish=250, perturb=(3.0, 8.0), repack=False, cpu=1, seed=None, verbose=False)` | Docks a small molecule into a receptor, or refines the placement of one protein chain against another. With `pose2` a `Molecule`, ligand docking follows AutoDock Vina: the receptor is turned into one affinity map per ligand atom type by scoring a one-atom probe with `Score('AutoDock Vina')` at every grid point of the `size` Å box at `centre` (default the ligand's own centroid) with `spacing` Å, `n_runs` independent Monte Carlo searches with local optimisation after every move run on the maps for `n_steps` steps each (default Vina's count) at temperature `T` (default 1.2 kcal/mol), and the minima found are rescored, refined against the exact score by a quasi-Newton descent, a `polish`-step Metropolis walk and a second descent, then clustered at `min_rmsd` Å, so every reported affinity is a `Score()` value in kcal/mol. `maps=log['maps']` reuses the maps of a previous call on the same receptor and box. Ligand rings are rigid; the ligand must carry bond orders (SDF, MOL, MOL2). With `pose2` a `Pose`, or `pose2=None` and `chain=` naming the moving chain(s) of `pose1`, it is a local rigid-body Monte Carlo refinement of the moving chain scored by the energy of the whole complex (`Score('REF15')` by default, any `Score` or `ForceField`) from a random `perturb` (Å, degrees) of the given placement, with `repack=True` alternating three rounds of repacking the side chains that line the interface, from the backbone-dependent rotamer library, with rigid-body minimisation, which is the cycle Rosetta's high-resolution docking runs (an integer sets the number of rounds); blind protein/protein docking is not attempted. `cpu` runs the independent work in that many processes with an identical result. Returns `(E_best, log)` with the ligand or moving chain left at the best pose, and `log['poses']` best first with `'coordinates'`, `'affinity'`, `'inter'`, `'intra'`, `'rmsd_to_best'`, `'run'`, `'cluster_size'`, plus `'interaction'` = E(AB) − E(A) − E(B) for a protein; also `'maps'`, `'energies'`, `'accepted'`, `'n_torsions'`, `'rotatable_bonds'`, `'box'`, `'n_evals_exact'`, `'n_evals_grid'`, `'time_maps'`, `'time_search'`, `'time_refine'`, `'complex'` |
