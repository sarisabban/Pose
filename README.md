...
| `PCR(sequence)`                                         | Generates ideal forward and reverse PCR primers for a DNA template (DNA only, accepts only A/C/G/T). Each candidate must satisfy length 18–25, GC content 40–60%, nearest-neighbor SantaLucia 1998 Tm in `[55, 65]` °C, a 3' GC clamp, no run of 4 identical bases, no internal palindrome (hairpin), no 3' self-dimer, and forward/reverse Tm within 2 °C of each other. Returns a tuple `(forward_string, reverse_string)` |
